...
class LedMode(IntEnum):
    ...
    Currently only RGB_TEMP is supported.
    """
    RGB_TEMP = 0x0d
...
class GoveeBluetoothLight(LightEntity):
    ...
    def __init__(self, hub: Hub, ble_device, config_entry: ConfigEntry) -> None:
        ...
        self._is_rgb_temp = self._model in RGB_TEMP_MODELS
        self._has_brightness = self._model in BRIGHTNESS_MODELS

        color_modes: set[ColorMode] = {ColorMode.ONOFF}

        if self._is_rgb_temp:
            color_modes.add(ColorMode.RGB_TEMP)
    ...
    async def async_turn_on(self, **kwargs) -> None:
        await self._send_command(prepareSinglePacketData(LedCommand.POWER, [0x1]))
        self._state = True

        if ATTR_BRIGHTNESS in kwargs:
            brightness = math.ceil(brightness_to_value(BRIGHTNESS_SCALE, kwargs[ATTR_BRIGHTNESS]))
            await self._send_command(prepareSinglePacketData(LedCommand.BRIGHTNESS, [brightness]))
            self._brightness = kwargs[ATTR_BRIGHTNESS]

        if ATTR_RGB_COLOR in kwargs:
            red, green, blue = (math.ceil(x) for x in kwargs[ATTR_RGB_COLOR])
            await self._send_command(prepareSinglePacketData(LedCommand.COLOR, [LedMode.RGB_TEMP, red, green, blue]))
            ...
            await self._send_command(prepareSinglePacketData(LedCommand.COLOR,
                                                          [LedMode.RGB_TEMP, red, green, blue,
    ...
    async def async_turn_off(self, **kwargs) -> None:
        await self._send_command(prepareSinglePacketData(LedCommand.POWER, [0x0]))
        self._state = False
