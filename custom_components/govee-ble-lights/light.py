...
class GoveeBluetoothLight(LightEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        commands = [prepareSinglePacketData(LedCommand.POWER, [0x1])]

        self._state = True

        if ATTR_BRIGHTNESS in kwargs:
            brightness = math.ceil(brightness_to_value(BRIGHTNESS_SCALE, kwargs[ATTR_BRIGHTNESS]))
            commands.append(prepareSinglePacketData(LedCommand.BRIGHTNESS, [brightness]))
            self._brightness = kwargs[ATTR_BRIGHTNESS]

        if ATTR_RGB_COLOR in kwargs:
            red, green, blue = kwargs.get(ATTR_RGB_COLOR)
            commands.append(prepareSinglePacketData(LedCommand.COLOR, [LedMode.MANUAL, red, green, blue]))
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await client.write_gatt_char(UUID_CONTROL_CHARACTERISTIC,
                                     prepareSinglePacketData(LedCommand.POWER, [0x0]), False)
        self._state = False
